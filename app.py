...
def split_value_unit(raw: object) -> tuple[str,str]:
    ...
    if m:
        ...
        if "/" in unit or "-" in unit: return s, ""
        return val, unit
...
with tab1:
    ...
    attr_file = st.file_uploader("อัปโหลดไฟล์ Attributes (xlsx)", type=["xlsx","xls"], key="tab1_attr")
    ...
    if st.button("ประมวลผลภาพ", key="btn_tab1") and attr_file and imgs:
        ...
        for r in results:
            d = r["extracted_data"]
            fn = r["file_name"]
            asset = d.get("ASSETNUM","")
            site  = d.get("SITEID","")
            code  = d.get("POWTR_CODE","")
            if "error" in d or "raw_text" in d:
                rows.append({"file_name":fn,"ASSETNUM":asset,"SITEID":site,
                             "POWTR_CODE":code,"ATTRIBUTE":"Error","VALUE":d.get("error")})
            else:
                for k,v in d.items():
                    if k in ("ASSETNUM","SITEID","POWTR_CODE"): continue
                    rows.append({"file_name":fn,"ASSETNUM":asset,"SITEID":site,
                                 "POWTR_CODE":code,"ATTRIBUTE":k,"VALUE":v})
    ...
    if st.button("ประมวลผล validated", key="btn_tab2") and attr2 and val_file:
        ...
        for _,r in dfv.iterrows():
            ...
            if prev and asset!=prev:
                rows.append({"file_name":"","ASSETNUM":"","SITEID":"",
                             "POWTR_CODE":"","ATTRIBUTE":"","VALUE":"","MEASUREUNIT":""})
            prev=asset
            for attr in canonical:
                ...
                rows.append({
                    "file_name":r.get("Plant",""),"ASSETNUM":asset,"SITEID":site,
        ...
        buf.seek(0)
        st.download_button("ดาวน์โหลด extracted_long_from_validated.xlsx",
                           buf,"extracted_long_from_validated.xlsx")
...
with tab3:
    ...
    if uploaded:
        ...
        buf.seek(0)
        st.download_button("Download validated_powtr_codes.xlsx",
                           buf,"validated_powtr_codes.xlsx")
...
with tab4:
    ...
    if st.button("ประมวลผล Nameplate", key="btn_tab4") and imgs4:
        prompt4 = (
            ...
            "ในรูปนี้ แล้วจัดให้อยู่ใน JSON โดยใช้ key เป็น field ภาษาอังกฤษ "
            "และ value เป็นค่าที่อ่านได้"
        )
        ...
        for f in imgs4:
            ...
            for k,v in js.items() if isinstance(js,dict) else [("error",js)]:
        ...
        buf.seek(0)
        st.download_button("ดาวน์โหลด nameplate_extracted.xlsx",
                           buf,"nameplate_extracted.xlsx")
